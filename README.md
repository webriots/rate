...
func main() {
    ...
    // - 1024 buckets (must be power of 2)
...
The `numBuckets` parameter must be a power of two (e.g., 256, 1024, 4096) for efficient hashing and is a critical factor in the limiter's performance and fairness.
...
limiter, err := rate.NewTokenBucketLimiter(
    65536,    // numBuckets (2^16) to maintain <10% collision probability
    10,       // burstCapacity
    100,      // refillRate
    time.Second,
)
